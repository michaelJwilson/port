...
Issue #59 item 5. `cnamaste.icm.icm_sweep_deque` takes fifteen parameters.
...
**Which `icm_sweep_deque`.** `cnamaste.icm` defines that name four times, at
...
class CsrGraph:
    ...
    `cnamaste` carries the same graph as three arrays through every call in
...
class IcmResult:
    ...
    `cnamaste` returns a bare `(niter, cost)` tuple, and the call site unpacks
...
def fold_unary(
    ...
    Notes
    -----
    **Bitwise, not approximately.** `cnamaste` computes
...
def icm_sweep(
    ...
    *,
    ...
    """Run `cnamaste`'s live sweep through the reduced interface.

    A delegation, not a reimplementation: the solver is `cnamaste`'s, so the
    ...
    beta : float
        The spatial coupling, `spatial_weight / temp` in `cnamaste`'s terms.
        One number because the solver uses one number.
    assignment : np.ndarray
        The starting labelling, **updated in place** -- `cnamaste`'s
    ...
    `min_clone_spots`). That is `cnamaste`'s, and it is neither seeded nor
