...
**`cnamaste` supports layouts it cannot produce, and the support is the
...
`run_cnamaste` never turns it on. Supporting it here would mean every consumer
...
def state_vector(parameters: Any, name: str = "a state parameter") -> np.ndarray:
    ...
    `cnamaste` agrees on what a second column would mean, so the one reading
...
def clone_path(
    ...
    if array.ndim > 1 and array.shape[1] != 1:
        msg = (
            ...
            f"`deconcatenate_clones` is off on every run `run_cnamaste` makes"
        )
        raise ValueError(msg)
