...
Issue #68. `run_cnamaste` starts at SNPs and genes; `tests/sim/truth.py` plants
at bins. Inverting `cnamaste`'s binning statistically is not possible -- it
sums -- but it does not have to be: a **pre-image** is enough. `unsegment`
constructs gene-level and block-level counts that `cnamaste`'s own aggregation
...
def unsegment(
    ...
    *,
    ...
    flip_every : int
        ...
        **Zero flips none.** The files `run_cnamaste` reads carry allele counts
        and not a phase, and the phase is `cnamaste`'s to infer, so a fixture
