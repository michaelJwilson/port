"""Write a pre-image to the files `cnamaste.io.load_input_data` reads (#68).

`tests/sim/unsegment.py` takes a binned fixture back to genes and blocks. This
...
the files `run_cnamaste` is pointed at.
...
from sim.unsegment import Unsegmented
...
`create_bin_ranges`' 5 Mb cap. It is what makes `cnamaste`'s own binning land
...
def write_genetic_map(truth: Truth, path: Path) -> Path:
...
def write_tmp_inputs(truth: Truth, pre_image: Unsegmented, root: Path) -> WrittenInputs:
...
def written_config(written: WrittenInputs) -> Iterator[None]:
    """Install the global config `cnamaste` reads, and put back what was there.
...
def load_written(written: WrittenInputs) -> Any:
    """Run `cnamaste.io.load_input_data` against what was written."""
    from cnamaste.io import load_input_data
