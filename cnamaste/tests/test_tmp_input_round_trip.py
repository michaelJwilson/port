...
`tests/sim/unsegment.py` builds a pre-image at the gene and block level;
`tests/sim/inputs.py` writes it as the files `run_cnamaste` is pointed at. This
closes the loop over `cnamaste.io.load_input_data` -- the real entry point,
...
import numpy as np
import pytest
from sim.inputs import load_written, write_tmp_inputs
...
def _written(tmp_path: Path, n_obs: int = 20) -> tuple[Any, Any, Any]:
    # NB two reads per neutral bin at a unit library factor: sparse enough
    #    that the partition leaves some genes all zero, which the loader's
    #    expression filter then has to drop.
    truth = planted(
        n_clones=2,
        n_states=3,
        lattice=(6, 5),
...
def test_the_loaded_files_bin_back_to_the_planted_fixture(tmp_path: Path) -> None:
    ...
    which is the one `cnamaste` reads into the channel it scores as B. The expression channel is binned from
