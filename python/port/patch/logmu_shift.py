...
On the `port` branch the call is live but its return is never read
(`# TODO fold in logmu_shifts`), so this replaces a function whose result
reaches nothing. Installing it would not change any run -- but *folding the
shift into the emission* would, because it debiases `log_mu` and every
...
truth as its referee rather than a loop whose answer is discarded.
`tests/test_clone_stack.py::test_the_shift_is_computed_and_then_discarded`
...
`num_segments_clones` and walks them individually, which admits unequal clones. A
...
def shifts(
    ...
    num_segments_clones: Sequence[int] | np.ndarray,
) -> np.ndarray:
    ...
    if lengths.size and bool(np.all(lengths == lengths[0])):
        ...
        return stack.per_clone(lambda view: scipy.special.logsumexp(view, axis=1))
    ...
    for clone, length in enumerate(lengths):
        ...
        out[clone] = scipy.special.logsumexp(terms[start:stop])
        start = stop
