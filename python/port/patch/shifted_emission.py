...
`port.patch.clone_shift`.

**Off by default, because `CLAUDE.md` forbids a silent behaviour change and
this is one.** Off, the call goes straight to `port.patch.coded_emission`,
...
class ShiftedEmission:
    ...
    A mixin, ahead of `port.patch.coded_emission.CodedEmission` in the MRO. It
    ...
    def _clone_state_weights(
        self,
        posteriors: np.ndarray,
        normal_log_lambda: np.ndarray,
        lengths: tuple[int, ...],
    ) -> np.ndarray:
        """`log W`, rebuilt only when the posterior behind it changes.

        `optimize_params` writes `self.state_posteriors` once per EM
        callback and then runs the optimizer over it, so the identity of that
        array is exactly the M step's boundary. Keyed on it, and holding a
        reference so the id cannot be reused underneath the entry.
        """
        cached = getattr(self, "_weight_cache", None)
        key = (id(posteriors), id(normal_log_lambda), lengths)

        if cached is None or cached[0] != key:
            cached = (
                key,
                (posteriors, normal_log_lambda),
                clone_state_weights(posteriors, normal_log_lambda, lengths),
            )
    ...
    def compute_emission_probability_nb_betabinom_coded(
        ...
        # NB the theta-free half is cached against the posterior that
        #    produced it, so the `(n_states, n_segments)` reduction happens
        #    once per M step and the per-call cost is `(n_clones, n_states)`.
        #    Recomputing it here instead cost 82.6 ms against 22.0 at
        #    10 clones x 40,000 x 7 -- the shift's whole affordability.
        shifts = log_normalizers_from_weights(
            self._clone_state_weights(posteriors, normal_log_lambda, lengths), log_mu
        )
