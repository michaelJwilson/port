...
- `TEMPERED` (forward-backward at falling temperature) reaches Viterbi's
  answer on every fit, 6-12x slower;
- `states="fit"` (the continuous fit's states, pairs as M-step parameters)
  cannot hold more planted pairs than the continuous fit has states, and
  `distinct` loses a state where that fit splits neutral bins across two;
- `dispersion="poisson"` and `"relax"` match `"fit"` on the pure fixtures and
  do not help the admixed ones, whose depth is overdispersed by the tumour
  expression profile, not by the start.
