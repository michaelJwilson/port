...
- `space="states"`, `cnaster`'s continuous fitted states as the pure
  profiles: a uniform normal admixture is absorbed into the fitted states,
  so it never reaches `W`; the lattice is what makes it visible.
- `ROW_MODE = "bic"`, nested support with a `0.5 log(bins)` cost per
  contaminant: ARI within 0.001 of entropy on the easy and hard admixed
  samples (0.9899 against 0.9908; 0.9830 against 0.9838) and cleaner `W`
  (no tumour-to-tumour weight); not adopted, the entropy regularizer being
  the simpler rule at equal recovery.
- `cap` and `anneal`, a bound `sum_{j != i} W_ij <= cap`, fixed or linear
  over a stage's outer iterations: extra hyperparameters with no recovery
  measured to need them.

Neither regularizer recovers planted per-clone normal fractions (0.04,
0.08, 0.12 on a 1,200-spot crop of the easy sample): the diploid column
reads 0.10, 0.087, 0.040 under BIC and 0.0, 0.063, 0.026 under entropy,
while `copy_likelihood.lattice_decode`'s tumour fractions come to 0.911
and 0.861 for the last two (0.92, 0.88 planted). Seeding that decode with
`1 - W[c, diploid]` changed neither its fractions nor copy ARI (0.9788).

The module is the package module as it stood before the split, so the
probes in `probes/` run every variant through the same wrapper.
"""
