...
| Registered markers | A test that does not name its referee (`upstream`, `subject`, `planted`, `property`, `analytic`, `exact`, `upstream_oracle`) or its type (`infra`, `end2end`, `oracle`, `equivalence`, `bug`, `warning`, `cnaster`), plus the tiers `benchmark`, `release`, `preprocessing` and `critical`. The type decides whether a test counts toward coverage (#149) |
